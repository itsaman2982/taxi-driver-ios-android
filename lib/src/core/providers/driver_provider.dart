...
class DriverProvider extends ChangeNotifier {
  ...
  Future<void> refreshProfile() async {
    try {
      ...
      AppLogger.error('Error refreshing driver profile', e);
    }
  }
  ...
  Future<void> toggleOnlineStatus(bool isOnline) async {
    try {
      ...
      if (response is Map && response['success'] == true) {
        ...
        AppLogger.info('Online status updated: $isOnline');
      }
    } catch (e) {
  ...
  Future<void> updateLocation(double lat, double lng,
    ...
    try {
      ...
      if (_lastHttpUpdate == null ||
          now.difference(_lastHttpUpdate!) > const Duration(seconds: 5)) {
        _lastHttpUpdate = now;
  ...
  Future<Map<String, dynamic>?> fetchPendingRide() async {
    try {
      ...
      AppLogger.error('Error fetching pending ride', e);
    }
    return null;
  }
  ...
  Future<Map<String, dynamic>?> checkCurrentRide() async {
    try {
      ...
      AppLogger.error('Error checking current ride', e);
    }
    return null;
  }
  ...
  Future<bool> acceptEmergency(String breakdownId) async {
    try {
      final res = await ApiService()
          .post('breakdowns/$breakdownId/accept-emergency', {});
      return res['success'] == true;
    } catch (e) {
      AppLogger.error('Error accepting emergency', e);
      return false;
    }
  }
  ...
  Future<bool> arrivedAtBreakdown(String breakdownId) async {
    try {
      final res =
          await ApiService().post('breakdowns/$breakdownId/arrived', {});
      return res['success'] == true;
    } catch (e) {
      AppLogger.error('Error reporting arrival at breakdown', e);
      return false;
    }
  }
  ...
  Future<bool> updateRideStatus(String rideId, String status) async {
    try {
      final res =
          await ApiService().post('rides/$rideId/status', {'status': status});
      return res['success'] == true;
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> reportBreakdown(
      String rideId, double lat, double lng, String notes) async {
    try {
      ...
      AppLogger.error('Error reporting breakdown', e);
      return null;
    }
  }
  ...
  Future<bool> cancelBreakdown(String rideId) async {
    try {
      ...
      AppLogger.error('Error cancelling breakdown', e);
      return false;
    }
  }
}
